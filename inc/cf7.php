...
{
	$GLOBALS['alergobot_cf7_render_context'] = [
...
{
	if (is_array($value)) {
		$value = array_filter($value, static fn($item) => $item !== '' && $item !== null);
...
function alergobot_cf7_format_agree_value($value)
{
	...
	if ($value === '1' || strcasecmp($value, 'on') === 0 || strcasecmp($value, 'yes') === 0) {
...
function alergobot_cf7_build_telegram_message()
{
	...
	foreach ($posted as $key => $raw_value) {
		...
		$label = 'ℹ️ ' . ucwords(str_replace(['-', '_'], ' ', $key));
		$lines[] = $label . ': ' . $value;
	}
 ...
 */
add_filter('wpcf7_posted_data', function ($posted) {
	$referer = wp_get_referer();

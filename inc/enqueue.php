...
add_action(
	...
	function () {
		...
		wp_enqueue_style( 'alergobot-swiper', $uri . '/css/libs/swiper-bundle.min.css', array(), $ver );
		wp_enqueue_style( 'alergobot-fancybox', $uri . '/css/libs/fancybox.min.css', array(), $ver );
		wp_enqueue_style( 'alergobot-reset', $uri . '/css/reset.min.css', array(), $ver );
		wp_enqueue_style( 'alergobot-global', $uri . '/css/style-global.min.css', array( 'alergobot-reset' ), $ver );
		wp_enqueue_style( 'alergobot-header', $uri . '/css/header.min.css', array( 'alergobot-global' ), $ver );
		wp_enqueue_style( 'alergobot-footer', $uri . '/css/footer.min.css', array( 'alergobot-global' ), $ver );

		// fallback при неполной сборке split CSS.
		if ( file_exists( ALERGOBOT_DIR . '/assets/css/style.min.css' ) ) {
			wp_enqueue_style( 'alergobot-style-bundle', $uri . '/css/style.min.css', array( 'alergobot-reset' ), $ver );
		}

		alergobot_enqueue_conditional_styles( $uri, $ver );

		wp_enqueue_script( 'alergobot-swiper', $uri . '/js/libs/swiper-bundle.min.js', array(), $ver, true );
		wp_enqueue_script( 'alergobot-fancybox', $uri . '/js/libs/fancybox.umd.js', array( 'alergobot-swiper' ), $ver, true );
		wp_enqueue_script( 'alergobot-app', $uri . '/js/app.min.js', array( 'alergobot-fancybox' ), $ver, true );

		$map = function_exists( 'alergobot_get_map_settings' ) ? alergobot_get_map_settings() : array();
...
function alergobot_enqueue_conditional_styles( $uri, $ver ) {
	$deps = array( 'alergobot-global', 'alergobot-header', 'alergobot-footer' );

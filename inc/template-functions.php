...
if ( ! function_exists( 'alergobot_query_blogs' ) ) {
	function alergobot_query_blogs( $args = array() ) {
		$defaults = array(
			'post_type'           => 'blogs',
			'post_status'         => 'publish',
			'posts_per_page'      => 6,
...
if ( ! function_exists( 'alergobot_get_blog_badge_label' ) ) {
	function alergobot_get_blog_badge_label( $post_id = 0 ) {
		$post_id = $post_id ?: get_the_ID();
		$terms   = wp_get_post_terms( $post_id, 'blog_category', array( 'fields' => 'slugs' ) );
...
if ( ! function_exists( 'alergobot_get_blog_archive_context' ) ) {
	function alergobot_get_blog_archive_context() {
		$context = array(
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Public archive pagination query arg.
		...
		if ( is_tax( 'blog_category' ) ) {
			...
			if ( $term instanceof WP_Term ) {
				...
				$context['active_tab']        = 'novosti' === $term->slug ? 'news' : 'articles';
			}
		} elseif ( is_tag() ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Public archive tab query arg.
		} elseif ( isset( $_GET['tab'] ) && 'news' === sanitize_key( wp_unslash( $_GET['tab'] ) ) ) {
...
if ( ! function_exists( 'alergobot_get_blog_archive_context_from_request' ) ) {
	function alergobot_get_blog_archive_context_from_request( array $overrides = array() ) {
		$context = alergobot_get_blog_archive_context();
		$tab     = isset( $overrides['tab'] ) ? sanitize_key( $overrides['tab'] ) : '';

		if ( 'news' === $tab || 'articles' === $tab ) {
...
if ( ! function_exists( 'alergobot_get_blog_archive_base_query_args' ) ) {
	function alergobot_get_blog_archive_base_query_args( array $context, array $extra = array() ) {
		$args      = array_merge( array( 'paged' => 1 ), $extra );
		$tax_query = array();

		if ( 'blog_category' === $context['archive_taxonomy'] && $context['archive_term_id'] ) {
			$tax_query[] = array(
...
if ( ! function_exists( 'alergobot_get_blog_archive_query_args' ) ) {
	function alergobot_get_blog_archive_query_args( array $context, $tab = 'articles', array $extra = array() ) {
		$args = array(
			'posts_per_page' => $context['posts_per_page'],
			'paged'          => 'news' === $tab ? $context['news_paged'] : $context['articles_paged'],
		);

		$tab_slug      = 'news' === $tab ? 'novosti' : 'stati';
		$tab_term_id   = 'news' === $tab ? $context['news_term_id'] : $context['articles_term_id'];
		$archive_match = 'blog_category' === $context['archive_taxonomy']
...
if ( ! function_exists( 'alergobot_render_blog_pagination' ) ) {
	function alergobot_render_blog_pagination( WP_Query $query, array $args = array() ) {
...
if ( ! function_exists( 'alergobot_get_related_blogs_query' ) ) {
	function alergobot_get_related_blogs_query( $post_id = 0, $limit = 2 ) {
		...
		$categories = wp_get_post_terms( $post_id, 'blog_category', array( 'fields' => 'ids' ) );
...
if ( ! function_exists( 'alergobot_get_product_category_terms' ) ) {
	...
	function alergobot_get_product_category_terms() {
		$order = array( 'ustroystva', 'analizatory', 'reagenty', 'paneli' );
		$terms = get_terms(
			array(
...
if ( ! function_exists( 'alergobot_resolve_catalog_teaser_gallery_item' ) ) {
	/**
	 * Normalize catalog-teaser gallery row: image URL, alt, dimensions.
	 *
	 * @param array $item ACF gallery repeater row.
	 * @return array{img_url: string, img_alt: string, img_w: int|string, img_h: int|string}|null
	 */
...
if ( ! function_exists( 'alergobot_render_catalog_gallery_product' ) ) {
	...
	function alergobot_render_catalog_gallery_product( array $item, $tag = 'a' ) {
		...
		<span class="catalog__product-media">
			<img class="catalog__product-img<?php echo $has_hover ? ' catalog__product-img--default' : ''; ?>" src="<?php echo esc_url( $item['img_url'] ); ?>" alt="<?php echo esc_attr( $item['img_alt'] ); ?>" title="<?php echo esc_attr( $item['img_alt'] ); ?>"<?php echo $img_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Built from esc_attr() fragments. ?> loading="lazy">
			<?php if ( $has_hover ) : ?>
				<img class="catalog__product-img catalog__product-img--hover" src="<?php echo esc_url( $item['img_hover_url'] ); ?>" alt="" aria-hidden="true"<?php echo $img_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Built from esc_attr() fragments. ?> loading="lazy">
			<?php endif; ?>
		</span>
		<?php
		echo 'a' === $tag ? '</a></div>' : '</div></div>';
	}
}
...
if ( ! function_exists( 'alergobot_split_faq_columns' ) ) {
	function alergobot_split_faq_columns( array $items ) {
		...
		return array( array_slice( $items, 0, $mid ), array_slice( $items, $mid ) );
	}
}

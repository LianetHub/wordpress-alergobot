		...
		<div class="pick__header">
			<div class="pick__main">
				<?php if ( $title = alergobot_home_get( 'title' ) ) : ?>
					<h2 class="pick__title title title-md <?php echo alergobot_anim_class( 'fade-right' ); ?>"><?php echo wp_kses_post( $title ); ?></h2>
				<?php endif; ?>
				<?php if ( $text = alergobot_home_get( 'text' ) ) : ?>
					<p class="pick__text text-lead <?php echo alergobot_anim_class( 'fade-up' ); ?>"><?php echo esc_html( $text ); ?></p>
				<?php endif; ?>
			</div>
				...
				<?php if ( alergobot_home_rows( 'tags' ) ) : ?>
					<div class="pick__tags">
						<?php foreach ( alergobot_home_rows( 'tags' ) as $tag_item ) : ?>
							<span class="pick__tag tag <?php echo alergobot_anim_class( 'scale-up' ); ?>"><?php echo esc_html( $tag_item['text'] ?? '' ); ?></span>
						<?php endforeach; ?>
					</div>
				<?php endif; ?>
				<div class="pick__benefits">
					<?php if ( $benefits_title = alergobot_home_get( 'benefits_title' ) ) : ?>
						<h3 class="pick__subtitle <?php echo alergobot_anim_class( 'fade-up' ); ?>"><?php echo esc_html( $benefits_title ); ?></h3>
					<?php endif; ?>
					<?php if ( alergobot_home_rows( 'benefits_list' ) ) : ?>
						<ul class="pick__list <?php echo alergobot_anim_class( 'stagger' ); ?>">
		...
		<?php if ( alergobot_home_rows( 'videos' ) ) : ?>
			<div class="pick__videos">
				<?php foreach ( alergobot_home_rows( 'videos' ) as $video ) : ?>
					<div class="pick__video <?php echo alergobot_anim_class( 'zoom' ); ?>">
						...
						<?php if ( ! empty( $video['captions'] ) && is_array( $video['captions'] ) ) : ?>
							<div class="pick__caption">
								<?php foreach ( $video['captions'] as $caption ) : ?>
									<p class="<?php echo alergobot_anim_class( 'fade-up' ); ?>"><?php echo esc_html( $caption['text'] ?? '' ); ?></p>
								<?php endforeach; ?>
							</div>
						<?php endif; ?>
					</div>
